...
Net::Domain - Perform domain lookups on 2-letter and 3-letter TLDs
...
=head1 SYNOPSIS
     ...
     $domain = new Net::Domain($domaintocheck);

     # Make sure domain name is valid one.
     @errors = $domain->Net::Domain::IsValid();

     # Returns availability and registrar information
     @response = $domain->Net::Domain::DoWhois(1);>

=head1 DESCRIPTION

Net::Domain performs domain lookups for 2-letter and 3-letter top level domains.  It also verifies the validity of
...
whois output from a single domain lookup, unless you know what registration agency to look at.  Net::Domain solves
...
Net::Domains requires that the Sockets (included with the Perl distribution) module is installed.

=head1 VERSION

Net::Domain Version 1.00  2/17/2000
